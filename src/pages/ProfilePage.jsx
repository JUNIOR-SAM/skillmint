        ...
        <div style={{ maxWidth: 900, margin: '0 auto', padding: '40px 24px' }}>
          <div className="profile-header-inner" style={{ display: 'flex', gap: 32, alignItems: 'flex-start' }}>
            ...
            <div className="profile-bio-block" style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
              ...
              {/* Social links */}
              {(profile.linkedin || profile.instagram || profile.portfolio || profile.facebook || profile.twitter || profile.tiktok) && (
                <div className="profile-socials" style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
                  {profile.linkedin  && <a href={profile.linkedin.startsWith('http') ? profile.linkedin : `https://${profile.linkedin}`} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: '#0077B5', textDecoration: 'none', padding: '5px 12px', borderRadius: 50, border: '1px solid rgba(0,119,181,0.3)', background: 'rgba(0,119,181,0.08)' }}>LinkedIn</a>}
                  {profile.instagram && <a href={`https://instagram.com/${profile.instagram.replace('@','')}`} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: '#E1306C', textDecoration: 'none', padding: '5px 12px', borderRadius: 50, border: '1px solid rgba(225,48,108,0.3)', background: 'rgba(225,48,108,0.08)' }}>Instagram</a>}
                  {profile.facebook  && <a href={profile.facebook.startsWith('http') ? profile.facebook : `https://${profile.facebook}`} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: '#1877F2', textDecoration: 'none', padding: '5px 12px', borderRadius: 50, border: '1px solid rgba(24,119,242,0.3)', background: 'rgba(24,119,242,0.08)' }}>Facebook</a>}
                  {profile.twitter   && <a href={`https://twitter.com/${profile.twitter.replace('@','')}`} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: '#1DA1F2', textDecoration: 'none', padding: '5px 12px', borderRadius: 50, border: '1px solid rgba(29,161,242,0.3)', background: 'rgba(29,161,242,0.08)' }}>Twitter/X</a>}
                  {profile.tiktok    && <a href={`https://tiktok.com/@${profile.tiktok.replace('@','')}`} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: '#ff0050', textDecoration: 'none', padding: '5px 12px', borderRadius: 50, border: '1px solid rgba(255,0,80,0.3)', background: 'rgba(255,0,80,0.08)' }}>TikTok</a>}
                  {profile.portfolio && <a href={profile.portfolio.startsWith('http') ? profile.portfolio : `https://${profile.portfolio}`} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: '#7B61FF', textDecoration: 'none', padding: '5px 12px', borderRadius: 50, border: '1px solid rgba(123,97,255,0.3)', background: 'rgba(123,97,255,0.08)' }}>Portfolio</a>}
                </div>
              )}
